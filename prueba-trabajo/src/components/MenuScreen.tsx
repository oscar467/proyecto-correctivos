...
import { MantenimientoCorrectivoIcon, IconoCheckList,MantenimientoPreventivoIcon, UserIcon } from './Icons'; // Importar desde Icons.tsx
...
const MenuScreen: React.FC<MenuScreenProps> = ({ username, companyName, onLogout, onNavigateToAlistamiento }) => {
  ...
  const menuOptions = [
    { label: 'Mantenimiento Preventivo', icon: <MantenimientoPreventivoIcon className="w-10 h-10" />, action: () => alert('Función no implementada'), implemented: false },
  ...
  return (
    <div className="w-full md:max-w-md lg:max-w-full lg:w-4/5  h-screen flex flex-col space-y-6 bg-white-100 overflow-hidden">
      <div className={`w-full h-full flex flex-col gap-4 bg-white rounded-2xl shadow-lg md:max-h-[calc(100%-5%)] lg:max-h-full lg:h-full ${isMobile ? 'mt-20' : ''}`}> {/* Aplicado max-h con calc */}
        {/* --- HEADER CARD --- */}
        <header className="bg-blue-500 text-white p-4 rounded-2xl flex items-center gap-4 shadow-lg flex-shrink-0">
          ...
          <div className="flex-grow md:w-full" >
            ...
            <p className="text-blue-100 text-sm flex flex-row items-center font-bold"><UserIcon className="w-8 h-8 mr-2" /> {username}</p>
          </div>
          <div className="flex-grow flex justify-end md:w-0 lg:w-full">
            <img className={`text-blue-600 rounded-full shadow-xl bg-white h-20 ${isMobile ? 'hidden' : 'block'}`} src="/logo-calisoft-sas.webp" alt="Logo_Calisoft" />
          </div>
        </header>
